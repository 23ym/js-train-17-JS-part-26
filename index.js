...
function replaceText(word, replacement, text) {
  const regex = new RegExp(word, 'gi'); // Створення регулярного виразу для пошуку слова з флагом 'g' (глобальний пошук).
  const replacedText = text.replace(regex, replacement); // Використання методу `replace` регулярного виразу для заміни слова на фразу у тексті.
...
function checkWord(word, text) {
  ...
  const isPresent = regex.test(text); // Використання методу `test` регулярного виразу для перевірки наявності слова у тексті.
  return isPresent; // Повернення результату перевірки.
}
...
function extractTextInParentheses(str) {
  ...
  const extractedTexts = matches.map(match => match[1]); // Створення масиву зі знайденими текстами.
...
function findWordOccurrences(str, word) {
  ...
  const matches = [];  // Створюємо пустий масив matches, та змінну match без значення
  let match; // За допомогою циклу whild створюємо ітерацію поки рядок містить збіги з регулярним виразом, та змінній match присвоюємо збіги
...
function checkRegexFlags(regex) {
  ...
  return flags.includes('g') && flags.includes('m'); // Перевіряємо наявність флагів 'g' та 'm' за допомогою методу `includes`.
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const result = str.replace(regex, newWord); // Заміняємо всі входження слова у рядку на нове слово.
...
function checkFlags(regex) {
  const usedFlags =[]; // Створюємо масив для зберігання використаних флагів.
  ...
  const sourceCode = regex.source;    // Отримуємо вихідний код регулярного виразу за допомогою властивості `source`.
  usedFlags.push(sourceCode); // Додаємо вихідний код до масиву
  return usedFlags; // Повертаємо масив використаних флагів.
}
...
function checkRegexMethods(regex) {
  const usedMethods = []; // Створюємо масив для зберігання використаних методів.
  if (regex.dotAll) {
    usedMethods.push('dotAll');  // Перевіряємо, чи використовується метод `dotAll`.
  }
  if (regex.multiline) {
    usedMethods.push('multiline'); // Перевіряємо, чи використовується метод `multiline`.
  }
  if (regex.sticky) {
    usedMethods.push('sticky');// Перевіряємо, чи використовується метод `sticky`.
  }
...
function findWord(str, word) {
  const regex = new RegExp(word, 'i');  // Створення регулярного виразу для пошуку слова.
